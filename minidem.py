...
# main parameters of the simulation that you need to set
max_iteration = 0
loop_function = None

# some useful global param
grain_list = []
iteration = 0
...
def animate(i):
    ...
    for grain in grain_list:
        if hasattr(grain, 'patch') is False:
            grain.patch = plt.Circle((grain.pos.x, grain.pos.y), grain.radius)
            ax.add_patch(grain.patch)
        grain.patch.center = (grain.pos.x, grain.pos.y)

    patch_list = [grain.patch for grain in grain_list]

    return patch_list


def run():
    anim = animation.FuncAnimation(fig, animate, frames=max_iteration, interval=50, blit=True)
    #plt.show()
    anim.save('im.mp4', metadata={'artist': 'Guido'})

# the enclidean 2D vector object. The following source comes from
